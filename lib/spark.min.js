...
(function($,progress,window){"use strict";if($&&progress&&typeof window.spark==="undefined"){var jsdoUsername=null;var jsdoPassword=null;var jsdoSession=null;var optionDefaults={id:"",filterQuery:"",data:{},success:false,error:false};window.spark={field:{},form:{},grid:{},loader:{},nav:{},notify:{},jsdoError:function(ev){var jsdo=((ev.xhr||{}).jsdo||{getErrors:function(){return[];}});var errorArray=jsdo.getErrors();var errors=[];$.each(errorArray,function(i,error){switch(error.type){case-1:var errObj=null;if(error.responseText){try{errObj=JSON.parse(error.responseText||"");}
...
if(jsdoSession.loginResult!==progress.data.Session.LOGIN_SUCCESS){var username=jsdoUsername||"anonymous";var password=jsdoPassword||"";return jsdoSession.login(username,password).then(function(){return window.spark.loadCatalogs(catalogURI);},function(){return"Unable to reach the REST adapter to establish a session.";});}else{var promise=$.Deferred();promise.resolve("loggedIn");return promise;}},loadCatalogs:function(catalogURI){jsdoSession.catalogLoaded={};if(typeof catalogURI=="string"){jsdoSession.catalogLoaded[catalogURI]=false;}else if(catalogURI instanceof Array){$.each(catalogURI,function(i,catalog){jsdoSession.catalogLoaded[catalog]=false;});}
var username=jsdoUsername||"anonymous";var password=jsdoPassword||"";return jsdoSession.addCatalog(catalogURI,username,password).then(function(session,result,responses){$.each(responses,function(i,response){jsdoSession.catalogLoaded[response.catalogURI]=(response.result?true:false);});return responses;},function(session,result,responses){console.log("Response:",responses);return"Failed to load catalog(s).";});},createJSDO:function(resourceName){var jsdo=null;$.each(jsdoSession.JSDOs,function(i,j){if(j.name===resourceName){jsdo=j;return;}});return(jsdo||(new progress.data.JSDO({name:resourceName})));},setCredentials:function(username,password){jsdoUsername=username||null;jsdoPassword=password||null;},clone:function(originalProps,newProps){return $.extend(true,{},originalProps,newProps);},toProperCase:function(string){return string.replace(/\w\S*/g,function(txt){return txt.charAt(0).toUpperCase()+txt.substr(1).toLowerCase();});},getJsdoSession:function(){return jsdoSession;},getMergedOptions:function(options){return this.clone(optionDefaults,options);},getQueryStringValue:function(key){var urlParams=window.location.search;if(urlParams===""){urlParams=window.location.href;}
...
var promise=jsdoObj.saveChanges();promise.done(function(jsdo,success,request){}).fail(window.spark.jsdoFailure);return promise;}};}})(jQuery,progress,window);(function($,window){"use strict";if($&&typeof window.spark!=="undefined"){window.spark=$.extend(window.spark,{clearPersistentObject:function(key){var storage=window.localStorage||null;if(storage&&storage.clearObject){storage.clearObject(key);return true;}else if(storage){delete storage[key];return true;}else{window.spark.setCookie(key,"");return true;}},setPersistentObject:function(key,value){var storage=window.localStorage||null;if(storage&&storage.getObject){storage.setObject(key,value);return true;}else if(storage){storage[key]=JSON.stringify(value);return true;}else{window.spark.setCookie(key,JSON.stringify(value));return true;}},getPersistentObject:function(key){var storage=window.localStorage||null;if(storage&&storage.getObject){return storage.getObject(key);}else if(storage){var storageValue=storage[key];if(typeof(storageValue)==="string"){return JSON.parse(storageValue);}}else{var cookieValue=window.spark.getCookie(key);if(typeof(cookieValue)==="string"){return JSON.parse(cookieValue);}}
...
return{};},getViewState:function(grid){return{columns:grid.columns,sort:grid.dataSource.sort(),filter:grid.dataSource.filter(),group:grid.dataSource.group()};},createReadOnlyEditor:function(){return function(container,options){container.text(options.model[options.field]);}},createMultiLookup:function(fieldOptions){return function(container,options){options.sparkEditor=spark.field.createMultiLookup($('<input data-bind="value:'+options.field+'"/>').appendTo(container),fieldOptions);setTimeout(function(){var data=options.model[options.field]||options.model.defaults[options.field];if(typeof(data)==="string"&&data.indexOf(",")>-1){data=data.split(",");}
