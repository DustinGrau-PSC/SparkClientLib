...
return null;}};}
if(window.spark&&kendo){window.spark.form={doOnEnter:function(selector,callback,target){if($(selector).length){$(selector).keypress(function(ev){if(ev.which===13){if(callback&&typeof callback==="function"){setTimeout(function(){callback.apply(target||this,[ev]);},100);}}});}},doOnSubmit:function(selector,callback,target){if($(selector).length){$(selector).on("submit",function(ev){ev.preventDefault();if(callback&&typeof callback==="function"){callback.apply(target||this,[ev]);}});}},getValidator:function(selector,options){if($(selector).length){if($(selector).getKendoValidator()&&!options){return $(selector).getKendoValidator();}
